...
class CreditNoteTemplate extends BasePdfTemplate {
  ...
  async createCreditNote(formData = {}, language = null) {
    ...
    // CREDIT NOTE IÇIN ÖZEL HEADER (TUANA TEKSTIL + Logo + Credit Note Date)
    this.drawCreditNoteHeader(page, pageWidth, y, formData);
    y -= 70;

    // CREDIT NOTE başlığı - dil desteği ile
    const creditNoteTitle = this.languageService.getText('creditNote', this.language);
    page.drawText(creditNoteTitle, {
      x: 55,
      y: y + 30,
      size: 20,
      font: this.font,
    ...
    this.drawCreditNoteFooter(page, pageWidth, y, formData);

    return this.pdfDoc;
  }

  drawCreditNoteHeader(page, pageWidth, y, formData) {
    ...
    // Invoice Number ve Credit Note Number bilgileri
    ...
    const creditNoteNumberLabel = this.languageService.getText('creditNoteNumber', this.language);
    page.drawText(`${creditNoteNumberLabel}: ${formData['CREDIT NOTE NUMBER'] || ''}`, {
      x: pageWidth - 183,
      y: y - 5,
      size: 7,
      font: this.fontBold,
  ...
  drawGoodsTable(page, pageWidth, y, formData) {
    ...
    for (let pageIndex = 0; pageIndex < Math.ceil(goods.length / itemsPerPage); pageIndex++) {
      ...
      if (pageIndex > 0) {
        ...
        const invoiceNumber = formData['INVOICE NUMBER'] || '';
        const descriptionOfGoodsLabel = this.languageService.getText('descriptionOfGoodsRegardingOrder', this.language);
        const goodsTitle = `${descriptionOfGoodsLabel}: "${invoiceNumber}" (${this.language === 'tr' ? 'Devamı' : 'Continued'})`;
    ...
    if (kdvEnabled && kdvOrani > 0) {
      ...
      const generalTotalLabel = this.languageService.getText('generalTotalVatIncluded', this.language);
...
SWIFT: TEBUTRIS 032`
  ...
  drawNotesSection(page, pageWidth, y, formData) {
    ...
    page.drawText(creditNoteExplanationLabel, {
      x: 55,
      y: noteY,
      size: 8,
      font: this.fontBold,
    ...
    if (formData['Notlar'] && formData['Notlar'].trim()) {
      ...
      const notLines = creditNoteMessage.split('\n');
...
module.exports = CreditNoteTemplate;
